...
export const builtinHosts = {
    bitbucket: {
        ...
        prod: ({ org, pkg, commit }) => `https://bitbucket.org/${org}/${pkg}/get/${commit}.tar.gz`,
    ...
    github: {
        ...
        prod: ({ org, pkg, commit }) => `https://github.com/${org}/${pkg}/archive/${commit}.tar.gz`,
        extractCommit: ({ lines, pkg }) => {
            const gitLine = lines.find((l) => l.includes(`/${pkg}.git`));
            if (gitLine) {
                const parts = gitLine.split("#");
    ...
    gitlab: {
        ...
        prod: ({ org, pkg, commit }) => `https://gitlab.com/${org}/${pkg}/-/archive/${commit}/${pkg}-${commit}.tar.gz`,
        extractCommit: ({ lines, pkg }) => {
            const gitLine = lines.find((l) => l.includes(`/${pkg}.git`));
            if (gitLine) {
                const parts = gitLine.split("#");
...
export function getPrivateDeps(packageJson, lockLines, host) {
    ...
    for (const depName of depsNames) {
        const deps = packageJson[depName];
        if (!deps)
            continue;
...
export function applyVersions(packageJsonText, privateDeps, host, org, newEnv) {
    ...
    for (const { pkg, commit } of privateDeps) {
        const newVersion = newEnv === "DEV"
            ? host.dev({ org, pkg })
            : host.prod({ org, pkg, commit });
        result = result.replace(new RegExp(`"${pkg}": "(.*)"`), `"${pkg}": "${newVersion}"`);
    }
    return result;
}
